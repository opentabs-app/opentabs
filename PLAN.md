...
| D9 | **Everything is written from scratch.** No third-party source is copied; grouping, ranking, parsing and UI are original. | Nothing carries another project's licence obligations, so the whole product can carry one licence (AGPL-3.0-or-later) with no attribution to track. |
