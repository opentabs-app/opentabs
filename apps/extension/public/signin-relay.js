/**
 * Relays a session from the sign-in page to the worker.
 *
 ...
 * no amount of waiting makes the globals appear. The sign-in origin serves an
 ...
 * check that actually matters — that this page is the sign-in page and not
...
(function () {
  ...
  window.addEventListener("message", function (event) {
    ...
    var d = event.data;
    // These two names are the sign-in page's, not ours: it posts them, and
    // the same page serves every product that signs in this way. Matching
    // them is a contract to keep, not a name to choose.
  ...
  // Named by the sign-in page too: it listens for this exact source before
  // it will post the session.
