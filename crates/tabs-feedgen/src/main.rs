...
/// The app catalogue, now empty on purpose.
///
/// It used to list the sibling products of the suite this was built in, and
/// editing it put a new one on every user's new tab within a day. That was
/// the release valve, and it is exactly what tied an independent tab manager
/// to a platform its readers have never heard of. The Web Apps group stays;
/// what goes in it is the reader's own, added in Settings.
///
/// The file keeps being written, and keeps its shape, because it is what the
/// extension prefers over its bundled copy: serving an empty list is how the
/// old suite leaves the cards of people who installed it already. Deleting
/// the endpoint instead would leave them on their last cached catalogue.
///
