...
echo "opentabs.app"
check "site answers 200"            "200" "$(curl -sS -o /dev/null -w '%{http_code}' https://opentabs.app/)"
check "www redirects to the apex"   "301" "$(curl -sS -o /dev/null -w '%{http_code}' https://www.opentabs.app/)"
check "privacy page is there"       "200" "$(curl -sS -o /dev/null -w '%{http_code}' https://opentabs.app/privacy.html)"
...
check "feeds allow cross-origin reads" "access-control-allow-origin: *" \
  "$(curl -sS -o /dev/null -D - https://opentabs.app/tabs/v1/apps.json | tr 'A-Z' 'a-z')"
...
check "an unknown pack 404s rather than serving the app" "404" \
  "$(curl -sS -o /dev/null -w '%{http_code}' https://market.opentabs.app/v1/packs/no-such-pack)"
check "a deep link survives a refresh" "200" \
  "$(curl -sS -o /dev/null -w '%{http_code}' 'https://market.opentabs.app/#/pack/anything')"
...
check "the sign-in element is served" "200" \
  "$(curl -sS -o /dev/null -w '%{http_code}' https://market.opentabs.app/openapps/openapps-login.js)"
...
check "sign-in page is served"      "200" \
  "$(curl -sS -o /dev/null -w '%{http_code}' https://auth.opentabs.app/signin)"
